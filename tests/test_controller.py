...
def contracts_check(controller: Controller):
    ...
    contracts = controller.model.read_contracts()
    assert contracts == []
    ...
    # read
    db_contract = controller.model.read_contract(new_contract)
    assert db_contract == new_contract

    # set Active + read all
    controller.confirm_contract(new_contract.id)
    new_contract = controller.model.read_contract(new_contract)
    assert new_contract.status == ContractStatus.ACTIVE

    # read draft
    closed_contracts = controller.model.read_contracts(filter=(Contract.status == ContractStatus.DRAFT))
    assert len(closed_contracts) == 0

    # read active
    active_contracts = controller.model.read_contracts(filter=(Contract.status == ContractStatus.ACTIVE))
    assert len(active_contracts) == 1
    print(f'{active_contracts=}')

    # add new + read all
    controller.model.create_contract(random.choice(randomdata.contracts))
    contracts = controller.model.read_contracts()
...
def projects_check(controller):
    ...
    projects = controller.model.read_projects()
    assert projects == []

    # read active contracts
    active_contracts = controller.model.get_active_contracts()
    assert len(active_contracts) == 1
    ...
    active_contracts = controller.model.get_active_contracts(project_id=new_project.id)
    assert len(active_contracts) == 1
